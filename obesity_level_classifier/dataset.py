...
Ejemplo de uso desde la terminal (estando en la raíz del proyecto):
    python obesity_level_classifier/dataset.py --input-path data/raw/obesity_estimation_modified.csv --output-path data/processed/obesity_estimation_cleaned.csv
    o
    make data
"""
...
from obesity_level_classifier.config import PROCESSED_DATA_DIR, RAW_DATA_DIR
...
@app.command()
def main(
    input_path: Path = RAW_DATA_DIR / "obesity_estimation_modified.csv",
    output_path: Path = PROCESSED_DATA_DIR / "obesity_estimation_cleaned.csv"
):
